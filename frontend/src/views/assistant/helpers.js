export const assistantQuestionOptions = [
  ...
  { label: '画像数据口径与缺口说明', value: 'portrait_data_governance' },
  { label: '教师近年成果结构概括', value: 'achievement_summary' },
  { label: '成果如何支撑画像', value: 'achievement_portrait_link' },
  { label: '成果如何支撑推荐', value: 'achievement_recommendation_link' },
  { label: '当前推荐概览', value: 'guide_overview' },
  { label: '项目指南推荐原因说明', value: 'guide_reason' },
  { label: '图谱链路与降级状态', value: 'graph_status' },
...
export const buildAssistantFallbackAnswer = (questionType, reason = '当前问答接口暂时不可用。') => ({
  ...
  boundary_notes: ['当前为页面级安全回退说明。', '权限不足、数据不足或接口异常时不会伪造答案。'],
  failure_notice: '问答异常已被拦截，页面仍可继续操作。',
  question_type: questionType,
  source_governance: {
    answer_mode: '前端安全回退',
    scope_label: '仅保留系统内页面级说明',
    verification_note: '当前未生成新的数据答案，请回到原页面继续验证。',
    degraded_flags: ['问答主链路当前已降级为安全说明。'],
    unavailable_flags: [],
  },
})
