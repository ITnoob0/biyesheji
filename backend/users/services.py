...
FORGOT_PASSWORD_CODE_TTL_SECONDS = 300
...
def is_valid_contact_phone(value: str | None) -> bool:
    return bool(PHONE_PATTERN.fullmatch(normalize_contact_phone(value)))


def build_forgot_password_cache_key(employee_id: str, reset_via: str) -> str:
    return f"users:forgot-password:{employee_id}:{reset_via}"


def generate_local_verification_code() -> str:
...
def ensure_password_change_notification(user, *, temporary_password: str | None = None, created_by_admin: bool = False) -> None:
    ...
    if existing:
        ...
        existing.content = (
            "当前账号密码为管理员初始化或重置后的临时密码，请先前往个人中心修改密码，并补全手机号与邮箱。"
        )
        existing.action_path = "/profile-editor/security"
        existing.action_query = {"source": "password_change_required"}
        existing.payload = payload
        existing.save(update_fields=["title", "content", "action_path", "action_query", "payload", "updated_at"])
        return
    ...
    create_user_notification(
        ...
        category=UserNotification.CATEGORY_PASSWORD_RESET_REQUEST,
        title=title,
        content="当前账号密码为管理员初始化或重置后的临时密码，请先前往个人中心修改密码，并补全手机号与邮箱。",
        action_path="/profile-editor/security",
        action_query={"source": "password_change_required"},
        payload=payload,
    )


def clear_password_change_notifications(user) -> None:
...
def build_bulk_import_template_xlsx(*, is_system_admin: bool, college_name: str = "") -> tuple[str, bytes]:
    if is_system_admin:
        ...
        rows = [
            ...
            ["", "", "", "", f"固定为当前学院：{fixed_college}", "教师账号请在系统中选择职称"],
        ]
...
def _normalize_import_rows(rows: list[list[str]]) -> list[dict]:
    ...
    for index, row in enumerate(data_rows, start=start_line):
        employee_id = _read_row_value(row, header_index_map, "employee_id", fallback_index=0)
        real_name = _read_row_value(row, header_index_map, "real_name", fallback_index=1)
        email = _read_row_value(row, header_index_map, "email", fallback_index=2)
        contact_phone = _read_row_value(row, header_index_map, "contact_phone", fallback_index=3)
        department = _read_row_value(row, header_index_map, "department", fallback_index=4)
        if not (employee_id or real_name or department or email or contact_phone):
            continue
        normalized.append(
            {
                "row_number": index,
...
def _build_header_index_map(header_row: list[str]) -> dict[str, int]:
    ...
    aliases = {
        ...
        "real_name": {"姓名", "教师姓名", "真实姓名", "name", "real_name"},
        "department": {"学院", "所属学院", "院系", "department", "college"},
        "email": {"邮箱", "个人邮箱", "联系邮箱", "email"},
        "contact_phone": {"联系电话", "手机号", "手机", "phone", "contact_phone"},
    }
    normalized_header = [str(item or "").strip().lower() for item in header_row]
    for idx, value in enumerate(normalized_header):
        for field, field_aliases in aliases.items():
