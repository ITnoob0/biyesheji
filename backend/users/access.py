...
SYSTEM_ADMIN_ONLY_MESSAGE = "当前账号没有系统管理员权限。"
TEACHER_MANAGEMENT_ONLY_MESSAGE = "当前账号不能执行教师账户管理操作。"
SELF_OR_ADMIN_MESSAGE = "当前账号只能访问自己的账户信息。"
MANAGEABLE_TEACHER_ONLY_MESSAGE = "系统管理员账户不属于教师账户管理范围。"
...
def get_user_college_id(user) -> str:
...
def is_manageable_teacher(user) -> bool:
    return bool(user and not is_system_admin_user(user))


def is_same_college_user(request_user, target_user) -> bool:
    request_college_id = get_user_college_id(request_user)
    target_college_id = get_user_college_id(target_user)
    return bool(request_college_id and target_college_id and request_college_id == target_college_id)


def can_access_teacher_scope(request_user, target_user) -> bool:
    if not is_authenticated_user(request_user) or not target_user:
        return False
    ...
    if is_college_admin_user(request_user):
        return bool(is_teacher_user(target_user) and is_same_college_user(request_user, target_user))
...
def ensure_system_admin_user(user, message: str = SYSTEM_ADMIN_ONLY_MESSAGE) -> None:
...
def ensure_self_or_admin_user(request_user, target_user, message: str = SELF_OR_ADMIN_MESSAGE) -> None:
    if can_access_teacher_scope(request_user, target_user):
        return
    raise PermissionDenied(message)
