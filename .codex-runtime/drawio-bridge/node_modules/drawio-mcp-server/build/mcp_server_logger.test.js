...
describe("create_logger", () => {
    let mock_send_logging_message;
    let mock_set_request_handler;
    let mock_server;
    beforeEach(() => {
    ...
    it("should return a Logger object with log and debug methods", () => {
        const logger = create_logger(mock_server);
    ...
    it("debug should call sendLoggingMessage with debug level and data", async () => {
        ...
        const set_level_handler = mock_set_request_handler.mock.calls[1][1];
    ...
    it("should not log and print error if an invalid log level is used directly", () => {
        ...
        expect(console_error_spy).toHaveBeenCalledWith("Internal Error: Invalid log level used: invalid_level");
    ...
    it("should support valid per-logger setLevels, and emit debug message", async () => {
        create_logger(mock_server);
        const set_levels_handler = mock_set_request_handler.mock.calls[0][1];
        const set_level_handler = mock_set_request_handler.mock.calls[1][1];
        await set_level_handler({
            method: "logging/setLevel",
            params: { level: "debug" },
    ...
    it("should reset per-logger level to default when null is provided", async () => {
        ...
        expect(mock_send_logging_message.mock.calls.some((call) => {
            const arg = call[0];
            return (arg.level === "debug" &&
                arg.logger === "logging" &&
                arg.data &&
