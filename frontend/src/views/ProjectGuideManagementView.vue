      ...
      <el-card v-if="showListCard" shadow="never" class="workspace-surface-card">
        ...
        <div class="summary-grid" v-if="summary">
          <div class="summary-card">
            ...
            <span>指南总数</span>
            <p>开放 {{ summary.open_count }} / 草稿 {{ summary.draft_count }}</p>
          </div>
          <div class="summary-card">
            <strong>{{ summary.deadline_warning_count }}</strong>
            <span>近 30 天到期</span>
            <p>归档 {{ summary.archived_count }} / 已截止 {{ summary.closed_count }}</p>
          </div>
          <div class="summary-card">
            <strong>{{ summary.config_coverage_count }}</strong>
            <span>已配置细化规则</span>
            <p>待整理开放项 {{ summary.stale_open_count }}</p>
          </div>
        </div>
        ...
        <el-table :data="guides" v-loading="loading" empty-text="暂无项目指南">
          ...
          <el-table-column label="操作" width="140" fixed="right">
            <template #default="{ row }">
              ...
              <el-button v-if="row.status !== 'ARCHIVED'" link type="warning" @click="updateGuideStatus(row, 'ARCHIVED')">归档</el-button>
              <el-button v-else link type="success" @click="updateGuideStatus(row, 'OPEN')">重新开放</el-button>
...
const startEdit = (row: ProjectGuideRecord) => {
  ...
  form.lifecycle_note = row.lifecycle_note
}
...
const updateGuideStatus = async (row: ProjectGuideRecord, status: GuideStatus) => {
  ...
  ElMessage.success(status === 'ARCHIVED' ? '项目指南已归档' : '项目指南已重新开放')
  await loadGuides()
}
