...
**Keywords:** teacher research portrait; research management system; academic graph; project recommendation; controlled question answering
...
与此同时，教育数字化转型已经从单纯的信息化覆盖转向数据驱动、业务贯通和智能辅助并重的阶段(祝智庭 and 胡姣 2022)。我国高校科研评价改革也强调回归创新质量、学术贡献与服务价值，不宜将单一论文指标直接等同于科研能力全貌(教育部 and 科技部 2020)。在这一背景下，学者画像、科研竞争力可视化和智能分析技术的发展，为高校科研场景下的数据整合、关系表达和辅助解释提供了新的技术基础(王杨 et al. 2020)。然而，如果脱离真实业务流程和工程边界，相关技术容易被空泛地描述为复杂智能平台，而难以在实际项目中落地。
...
![课题研究背景与问题域示意图](final-media/research-background.png)
...
在学者画像与科研评价方面，国内外研究已经围绕论文产出、被引频次、合作网络、研究主题和学术影响力等指标形成了较为系统的分析框架。国内研究中，已有工作基于论文数据构建学者学术状态及竞争力可视化系统，用以支持科研管理与辅助决策(王杨 et al. 2020)。国外研究则较早从专家发现与学术网络挖掘角度出发，探索如何根据文档集合识别领域专家以及如何在组织或学术网络中建模知识能力(Balog, Azzopardi, and Rijke 2006)(Husain et al. 2019)。在科研评价方面，H 指数等指标具有较强代表性(Hirsch 2005)，但后续研究也指出，科研生产力和学术影响力不宜被单一指标完全概括，而应结合多维证据综合分析(Abramo and D’Angelo 2014)(Siudem et al. 2020)。这些研究为教师科研能力画像提供了理论支撑，但多数工作偏重学术信息服务平台、专家发现或评价研究，较少直接面向校内科研治理场景构建完整业务系统。

在知识图谱领域，相关研究已经围绕知识表示、知识获取、知识融合和知识应用形成了较成熟的研究体系(Ji et al. 2021)。国内综述进一步从通用图谱、行业图谱和构建方法等角度总结了知识图谱的发展脉络(黄恒琪 et al. 2019)(张吉祥 et al. 2022)。在学术信息服务场景下，知识图谱能够较好地表达教师、论文、关键词、项目和合作学者之间的关系网络，并已被用于学术信息组织与服务(汤庸 et al. 2018)。针对学术社交网络的研究也表明，科研关系网络具有较强的结构分析价值(Tang et al. 2008)。但对于本科毕业设计项目而言，直接实现复杂图挖掘、社区发现和多跳推理成本较高，因此更适合采用轻量图谱展示与关系统计相结合的实现路线。

在推荐系统方面，传统的基于内容推荐、协同过滤推荐与混合推荐方法已经发展较为成熟(Adomavicius and Tuzhilin 2005)，国内综述也系统总结了推荐技术从传统方法向深度学习方法演进的过程(于蒙 et al. 2022)。与此同时，也有综述从更广义的信息过滤视角讨论推荐系统的整体发展(Lü et al. 2012)。不过，高校项目申报场景与电商或内容平台场景存在显著差异，其重点不是提升点击率，而是围绕教师研究方向、成果基础和指南条件进行规则约束下的可解释匹配。因此，采用规则增强型推荐路径，更符合当前项目规模、数据条件与业务需要。

在智能问答方面，问答系统研究已经从专用问答逐步扩展到开放域问答和多模态问答(闫悦 et al. 2023)。在开放域问答链路中，稠密检索等技术提升了证据召回能力(Karpukhin et al. 2020)，而检索增强生成进一步推动了外部知识与生成模型的结合(Lewis et al. 2020)。随着大语言模型快速发展，相关研究也开始系统总结大模型在语言理解、生成和任务适配中的共性能力(Zhao et al. 2023)。尽管这些研究为系统中的智能辅助模块提供了理论参考，但当前项目并未实现完整的开放域知识库检索与 RAG 全链路，而是采用系统内知识片段检索、模板化回答和可选模型生成的受控方案，以保证结果可验证、来源可追溯、异常可回退。
...
表 1.1 对相关研究与本文系统定位之间的关系进行了归纳。可以看出，现有研究分别在学者画像、知识图谱、推荐系统、智能问答和可视分析等方向形成了较丰富的方法基础，但较少将这些思路以克制、可解释且贴合高校科研治理流程的方式整合到同一业务系统中。本文的工作重点正是在真实工程边界内完成这种系统化整合。
...
Element Plus 为系统提供了表单、表格、标签、弹窗、消息提示等常用业务组件，使前端能够快速构建符合管理系统风格的交互界面。ECharts 则主要用于实现科研画像雷达图、趋势图、成果结构图、院系分布图和学院对比图等可视化模块。可视分析研究表明，图形化表达与交互分析在复杂数据理解和决策支持中具有重要作用(Keim et al. 2008)。在学术管理场景中，学者竞争力可视化系统的研究也说明了结构化图表和交互页面对科研状态分析具有较强支撑作用(王杨 et al. 2020)。同时，智能数据可视分析研究进一步强调了数据管理、智能交互与可视表达协同发展的趋势(骆昱宇 et al. 2024)。因此，本系统在前端设计中不仅关注页面可用性，也强调通过图表和信息卡片提升教师与管理员对系统结果的理解效率。

![系统前端工作台结构图](final-media/frontend-workspace.png)

图 2.1 展示了前端工作台的主要组织方式。系统并未将页面简单堆叠为若干独立入口，而是围绕教师资料维护、成果治理、画像分析、推荐辅助和管理统计等高频任务构建统一工作台，以降低用户在不同页面之间切换时的理解成本。
...
在关系扩展层面，系统保留 Neo4j 作为学术图谱增强能力的可选实现。当图数据库环境可用时，系统可优先从图数据库读取教师、论文、关键词和合作学者之间的关系；当图数据库不可用时，系统仍可回退到 MySQL 中的关系数据进行轻量图谱展示。知识图谱综述指出，图结构表达在关系建模、语义组织和知识服务方面具有天然优势(Ji et al. 2021)(黄恒琪 et al. 2019)；而面向学术信息服务的研究也表明，图谱化组织有助于学者网络和学术资源的结构化服务(汤庸 et al. 2018)。但在实际工程项目中，需要平衡功能增强与系统稳健性，因此本文采用了“Neo4j 优先、MySQL 回退”的策略。
...
表 2.1 反映了系统在数据存储层面的总体取舍：以 MySQL 承担主业务数据持久化，以 Neo4j 提供可选关系增强，以本地文件系统支撑轻量媒体资源管理。该组合既满足当前项目的真实实现范围，也为图谱展示和后续功能扩展预留了接口。

## 本地化 AI 与大模型应用

智能辅助模块是本系统的重要扩展能力之一。近年来，问答系统研究不断向开放域和多模态方向扩展(闫悦 et al. 2023)。在此基础上，稠密检索与检索增强生成为知识密集型任务提供了新的技术思路(Karpukhin et al. 2020)(Lewis et al. 2020)，而大语言模型研究则进一步推动了统一语言生成与任务适配能力的发展(Zhao et al. 2023)。但对于高校教师科研管理系统而言，完全开放的问答模式容易引入外部知识噪声、答案不可验证和系统边界不清等问题。
...
同样地，推荐模块并未采用重训练的深度推荐模型，而是结合研究方向、学科信息、成果活跃度、画像优势维度和指南规则配置进行规则增强型匹配。推荐系统相关研究已经形成较为完整的方法谱系(于蒙 et al. 2022)(Adomavicius and Tuzhilin 2005)，但在本项目场景下，更重要的是结果边界清晰、推荐理由可解释和人工规则可维护。因此，当前实现选择了更克制的规则增强路线，这种方案既符合项目的工程边界，也更适合本科毕业设计对真实性与可解释性的要求。
...
在业务流程层面，系统以成果治理为主入口。教师录入论文、项目、知识产权、教学成果和学术服务后，数据先进入草稿或待审核状态，由管理员完成审核，通过的数据再进入画像计算、图谱展示、推荐匹配和学院统计等链路。对于论文成果，系统还补充了校内合著教师搜索、认领邀请、接受或拒绝处理等协同治理流程。也就是说，成果中心、画像模块、图谱模块、推荐模块和问答模块之间并非彼此孤立，而是围绕教师科研数据形成联动关系。这种从数据治理走向画像分析与管理服务的业务链条，与当前学者竞争力可视化和高校科研评价回归多维证据的研究与政策导向是一致的(王杨 et al. 2020)(教育部 and 科技部 2020)。

![系统业务用例图](final-media/usecase-overview.png)

图 3.1 采用软件工程文档中常见的角色-用例表达方式，对教师、学院管理员和系统管理员三类参与者的职责边界进行了统一描述。图中既保留了成果录入、画像查看、推荐与问答等教师侧核心操作，也体现了审核治理、账号维护、看板统计与项目指南配置等管理侧任务，从而说明系统并非若干页面的简单拼接，而是围绕科研治理主链路组织起来的完整业务闭环。
...
其中，关系分析需求与学术信息服务领域中基于知识图谱组织学者关系、主题关系和成果关系的思路较为一致(汤庸 et al. 2018)；推荐与问答需求则分别受到推荐系统与智能问答研究的启发，但在本系统中均被约束为规则增强和受控辅助两类轻量实现(于蒙 et al. 2022)(闫悦 et al. 2023)。对于管理分析需求，其核心目标不是构建大型 BI 平台，而是通过可视化方式提高学院管理者对科研结构与趋势的理解效率(Keim et al. 2008)。
...
除功能需求外，系统还需要满足若干非功能需求。第一，系统需要具备良好的安全性，确保教师资料、成果数据和管理信息不会因越权访问而泄露。第二，系统需要具备较好的可解释性，特别是在科研画像、项目推荐和智能问答等功能中，应能够向用户明确说明结果依据和边界。第三，系统需要具备可维护性，能够通过统一文档、测试脚本和异常提示机制降低后续维护难度。第四，系统需要具备可用性，使教师和管理员能够在较低学习成本下完成主要业务操作。上述要求与学术可视分析强调的“可理解、可交互、可追踪”原则具有一致性(Keim et al. 2008)，也符合当前高校科研评价中强调结果不能脱离真实贡献与可核验依据的导向(教育部 and 科技部 2020)。

## 可行性分析

从技术可行性来看，本系统所使用的 Vue 3、Django、MySQL、ECharts 和 JWT 等技术均具有成熟生态，且已经在项目中实现稳定运行。Neo4j 和智能辅助模块均采取可选增强或受控回退方式，不构成系统主链路的强依赖，因此技术实现风险较低。这种“主链路稳定、增强链路可选”的实现方式，也与知识图谱和智能问答在真实系统中逐步扩展的工程化路线相一致(Ji et al. 2021)(闫悦 et al. 2023)。
...
系统整体采用前后端分离的分层架构，如图 4.1 所示。该架构以教师科研数据治理为主线，将页面展示、接口服务、业务处理、数据存储和智能辅助进行分层组织，从而保证教师端与管理端能够围绕同一套业务数据形成稳定联动。总体上，前端负责页面展示、交互控制和业务编排；接口层负责身份认证、参数校验和统一返回；业务服务层负责成果治理、画像计算、推荐分析、看板统计和图谱协调；数据层负责 MySQL 主存储与 Neo4j 可选增强；智能辅助层则负责系统内知识片段组织、答案生成和异常回退。该架构既吸收了可视分析与知识图谱服务中关于多层次信息组织的设计经验，也保持了面向业务落地的工程克制(Keim et al. 2008)(汤庸 et al. 2018)。
...
![系统总体架构图](final-media/system-architecture.png)
...
如图 4.2 所示，系统模块之间并非并列割裂，而是存在清晰的依赖关系。成果管理中心与教师基础资料共同决定画像计算的输入口径；画像结果又反向参与项目推荐与问答回答组织；推荐反馈、问答来源卡片、图谱分析和学院看板统计则进一步为教师与管理员提供解释和决策支持。这种“画像—推荐—问答—看板”的联动设计，与学者画像可视化、推荐系统解释化以及知识图谱服务化的发展方向相呼应(王杨 et al. 2020)(于蒙 et al. 2022)(汤庸 et al. 2018)。

![系统功能模块结构图](final-media/function-module.png)
...
结合当前实现，用户层包含自定义用户表、通知表与教师基础档案；成果层包含论文、项目、知识产权、教学成果和学术服务五类成果实体，并进一步扩展合作作者、论文关键词、成果认领、成果操作日志和论文专用操作日志等关系对象；分析层包含画像快照等对象；推荐层包含项目指南、学院实体、收藏记录和推荐历史快照；通知层则用于承载项目推送、成果认领提醒与密码重置申请等消息。该设计兼顾了关系型数据库的稳定性和知识图谱增强时对实体关系组织的需求(黄恒琪 et al. 2019)(汤庸 et al. 2018)。
...
![系统数据库 ER 图](final-media/database-er.png)

| 数据表 | 关键字段示例 | 主要用途 |
|:--|:--|:--|
| CustomUser | username, role_code, department | 存储教师、学院管理员和系统管理员账号基础信息 |
| TeacherProfile | discipline, research_interests | 存储教师扩展档案，服务画像、推荐和问答 |
| Paper | title, doi, status, citation_count | 存储论文成果及其审核状态、元数据和引用信息 |
...
表 4.4 给出了支撑系统主链路运行的核心数据表。从表结构组织可以看出，系统将“用户与权限”“成果与治理”“分析与推荐”“通知与反馈”拆分为相对独立但可关联的对象，既满足当前功能实现需要，也为后续功能扩展保留了清晰的数据边界。
...
表 4.5 所示的状态字段设计，使成果审核、认领处理和消息通知都具备了明确的生命周期表达。对于管理型系统而言，统一而克制的状态建模比堆叠复杂流程更重要，这也是本文在总体设计阶段强调“轻量实现、过程可追踪”的原因。
...
在认证方式上，系统以后端 JWT 认证为主，前端结合路由守卫和会话信息完成登录态保持。对于需要登录的接口，请求头统一携带 `Authorization` 字段和 `Bearer` 令牌；对于当前用户、教师画像、推荐和管理员统计等接口，后端进一步根据 `role_code` 和权限范围校验可访问的教师或学院数据。接口返回值以“业务数据 + 元数据 + 错误信息”三类内容组织，在成功情况下返回结构化数据，在失败情况下返回统一错误对象、请求编号和下一步建议。这种接口组织方式有助于提高系统内知识问答、推荐解释和统计分析的可调用性与可追踪性(闫悦 et al. 2023)(于蒙 et al. 2022)。
...
| 问答接口 | /api/ai-assistant/portrait-qa/ | 返回受控式画像问答结果、来源卡片和边界说明 |
| 看板接口 | /api/achievements/academy-overview/ | 返回学院级统计、排行、钻取与导出数据 |
| 图谱接口 | /api/graph/topology/<user_id>/ | 返回学术图谱节点、边和轻量分析结果 |

主要接口分类说明

表 4.6 体现了系统接口按业务域进行组织的设计思路。该方式有利于前后端联调时快速定位功能边界，也便于在不影响主链路的前提下对图谱、推荐和问答等增强模块进行独立维护。
...
表 4.7 说明系统并未将错误处理停留在简单状态码层面，而是尽可能补充可恢复性提示和下一步建议。这种统一错误结构既有助于前端做出一致反馈，也有助于在演示和答辩环境中快速定位异常来源。
...
数据治理模块以成果管理中心为核心。系统支持论文、项目、知识产权、教学成果和学术服务五类成果的统一录入与维护，并针对论文成果实现了更细粒度的字段组织，包括摘要、DOI、卷期页、引用次数、合作作者、作者位次和通讯作者标记等。为了提高论文录入效率，系统实现了 BibTeX 文件解析、预览校验和确认导入机制，使教师能够在确认字段映射结果后将文献信息批量导入系统。教师登录系统后可从统一工作台进入成果管理页面，如图 fig:login-page 和图 fig:achievement-page 所示，依次完成资料维护、成果录入和数据提交。该模块强调结果真实、口径可核验的设计思路，也与当前高校科研评价中反对单一指标化、强调真实贡献证据的政策导向相吻合(教育部 and 科技部 2020)。考虑到论文仍处于修订阶段，本章涉及的页面截图位置暂以实际启动验证后的本地前端访问链接占位，待最终答辩版再替换为对应运行截图。
...
如图 5.1 所示，成果治理模块可以抽象为一条清晰的数据流：教师与管理员分别从录入、审核两个外部入口参与业务；成果库、教师画像基线库以及项目指南与知识片段库共同构成后续分析的核心数据来源；经过治理的数据再流向画像、推荐、问答和学院看板等服务模块。这种“先治理、后分析、再反馈”的链路保证了系统输出始终建立在可核验、可追踪的数据基础上，更符合软件工程说明文档对数据边界与处理过程的表达要求。

![成果治理数据流图](final-media/system-dataflow.png)
...
为增强结果可解释性，系统在返回雷达图数据的同时，还提供维度解释、权重说明、证据标签和趋势分析结果。对于历史年份，系统支持画像快照持久化，以便后续进行跨年趋势比较；对于当前年份，则采用实时聚合方式反映最新成果对画像结果的影响。教师可在个人中心完善研究方向、学科和研究兴趣后进入画像页查看结果，页面展示形式如图 fig:profile-editor-page 与图 fig:portrait-page 所示。

从评分引擎的具体实现看，系统采用固定权重加权求和方式计算综合分值，对应后端中的 `fixed_weights_runtime_aggregation` 口径。其中，基础学术产出、经费与项目攻关、知识产权沉淀、人才培养成效、学术活跃与声誉、跨学科融合度的权重分别为 24%、20%、16%、14%、14% 和 12%。六个维度并非凭空命名，而是围绕教师科研成果管理中最易核验的业务证据进行组织：基础学术产出强调论文数量与被引表现，经费与项目攻关强调项目数量与累计经费，知识产权沉淀强调知识产权数量与转化情况，人才培养成效强调教学成果与协同论文，学术活跃与声誉强调学术服务、合作作者与引用表现，跨学科融合度则强调关键词覆盖与项目参与情况。相关研究指出，科研评价不宜被单一指标完全代表，而应从生产力、影响力和多维表现等角度综合把握(Hirsch 2005)(Abramo and D’Angelo 2014)(Siudem et al. 2020)，这也是本系统采用多维画像而非单指标排名的主要依据。

需要说明的是，表 5.2 中的权重和公式属于当前原型系统的业务型权重，其目的在于形成稳定、可解释、可实时返回的分析视图，而不是构建高校通用的人才评价标准。画像数据口径以审核通过的成果为主；对于校内合著认领成功或存在署名冲突但已确认的论文，系统也会纳入教师相关成果集合中参与画像计算。历史年份优先读取已落库快照，对应后端中的 `portrait-snapshot-v1`；若快照缺失，则由后端按历史年份回溯计算并立即固化。当前年份保持运行时聚合视图，对应 `portrait-runtime-v1`，用于反映新增成果带来的即时变化。该设计兼顾了历史稳定性与当前可观察性，更符合现有代码的真实状态。
...
项目推荐模块建立在项目指南管理和教师画像分析的基础之上。管理员可维护指南标题、发布单位、状态、级别、推荐标签、目标关键词、目标学科、资助强度、适用范围和截止时间等信息。系统当前支持 `BALANCED`、`KEYWORD_FIRST`、`DISCIPLINE_FIRST`、`WINDOW_FIRST`、`ACTIVITY_FIRST`、`PORTRAIT_FIRST` 和 `FOUNDATION_FIRST` 七类规则档位，并允许在默认配置之上附加轻量规则参数。教师访问推荐模块时，系统会基于研究方向、学科院系、近三年成果活跃度、画像优势维度和指南规则配置生成推荐分值，并输出推荐理由、命中标签、优先级和支撑记录。推荐页与问答辅助面板的典型呈现方式分别如图 fig:recommendation-page 和图 fig:assistant-page 所示。

当前推荐实现强调规则增强与可解释性，而非复杂的深度学习排序模型。后端会先构建教师特征包，特征来源包括研究方向、研究兴趣、学科院系标签、论文关键词、近三年成果活跃度以及画像优势维度，再结合项目指南的关键词、目标学科、申报窗口、资助信息和规则档位计算推荐分数。其中特征包中的“近三年活跃度”来源于近三年论文、项目、知识产权、教学成果和学术服务的累计数量，画像联动部分则读取当前教师画像总分及前三个优势维度。命中结果除推荐分外，还会同步生成推荐理由、解释维度、推荐标签、关键词命中、学科命中以及支撑记录，用于说明推荐结果的来源。系统同时保存推荐历史快照，并支持收藏、感兴趣、暂不相关、计划申报、已申报等轻量反馈信号，但这些反馈当前仅用于人工复盘和后续规则迭代，不直接训练复杂排序模型。这样既吸收了推荐系统研究中关于内容匹配、反馈信号和解释维度的思路，又避免了超出当前项目边界的复杂模型训练(Adomavicius and Tuzhilin 2005)(于蒙 et al. 2022)。
...
对于智能问答功能，系统优先围绕教师资料、成果聚合、推荐结果和学院统计构建内部知识片段，再通过关键词重叠方式检索相关证据；当前论文涉及的主接口为 `/api/ai-assistant/portrait-qa/`，其问题类型在后端被归并为画像总结、维度原因、成果摘要、画像数据治理、成果与画像联动、成果与推荐联动、图谱状态、指南概览、推荐原因和学院统计等类别。接口返回结果除了答案文本外，还会同步返回 `source_details`、`source_governance`、`boundary_notes` 和可回跳页面信息，用于说明答案来源、当前边界和是否存在降级情形。当可选模型可用时，系统可将这些证据组织为更自然的中文回答；当模型不可用或调用异常时，则回退为规则化说明结果，或者切换到可选的 `/api/ai-assistant/dify-chat/` 扩展链路。通过这种方式，系统在引入智能辅助能力的同时保持了结果边界清晰和来源可验证。该实现借鉴了问答系统、稠密检索和检索增强生成研究对“证据组织—答案生成”链路的启发，但在工程上保持了严格受控的范围(闫悦 et al. 2023)(Karpukhin et al. 2020)(Lewis et al. 2020)(Zhao et al. 2023)。
...
|:--|:--|:--|
| portrait_summary、portrait_dimension_reason | 解释教师画像总体结果与维度原因 | 画像维度、权重说明、阶段对比结果 |
| achievement_summary、portrait_data_governance | 解释成果统计与画像数据口径 | 审核通过成果、认领状态、快照边界 |
...
学术图谱模块主要围绕教师、论文、关键词、合作学者和其他科研成果之间的关系进行展示。系统在运行时优先尝试从 Neo4j 读取图谱结果；若图数据库不可用，则回退到 MySQL 关系数据。图谱分析部分当前主要提供合作网络概览、基于阈值划分的轻量合作圈层、合作者类型统计、主题热点分析和重点提示卡片，属于轻量关系挖掘能力，而非复杂图推理或社区发现平台。这与学术信息服务和知识图谱研究中强调的“关系组织优先于复杂推理”的落地路径较为一致(汤庸 et al. 2018)(张吉祥 et al. 2022)。
...
表 5.8 说明学院看板与学术图谱并不是孤立存在的展示模块，而是分别服务于宏观统计观察与关系网络解释两个层面。通过将二者与教师画像和成果治理共享统一口径，系统能够在不增加复杂算法负担的前提下支撑学院级分析需求。
...
在异常治理方面，系统接口优先返回结构化错误信息，包括错误码、错误描述、可恢复性提示和请求编号。对推荐、图谱和问答等增强链路，系统也设计了相应的回退逻辑，以保证在增强能力异常时，核心业务页面仍然可用。结合仓库中已有的启动前检查脚本 、演示数据恢复脚本 、主链路回归脚本 以及统一验证入口 ，项目已经形成较完整的开发自检、演示恢复与主链路回归机制。

此外，系统在账户侧还通过统一的 `role_code`、`role_label` 和 `permission_scope` 收拢角色边界，避免在多个模块中重复分散定义权限规则；在教师侧，当前登录态仍保持为会话级存储方案，以满足演示与答辩环境下的轻量使用需求；在管理员侧，则通过批量账户操作、密码重置、通知提醒和恢复脚本保障演示数据始终可回到稳定状态。上述措施虽然规模不大，但对于本科毕业设计项目而言，已经体现出较完整的工程化意识与可维护性考虑。
...
表 6.4、表 6.5 和表 6.6 分别从主功能、权限边界和异常回退三个角度验证了系统的可用性。测试结果表明，当前实现不仅能够支撑教师与管理员的核心业务流程，也能够在越权访问、增强链路异常和演示环境损坏等场景下维持基本稳定性。
...
在此基础上，本文进一步使用本地生成脚本对关键接口进行了单次耗时测量，测量方式基于恢复后的演示数据和 Django 测试客户端，不等同于公网部署环境下的压测结果，但能够反映当前原型系统在本地开发环境中的接口响应级别。表 6.7 给出了主要接口与权限场景的测量结果。
...
表 6.7 说明，在当前演示规模下，画像总览、项目推荐、画像问答和学院统计等核心接口均能够在可接受时间内返回结构化结果；而权限拒绝类接口则能快速给出 401 或 403 响应，有助于维持主链路的稳定性与边界清晰性。

![系统测试结果汇总图](final-media/test-result.png)
...
后续工作可从以下几个方面继续完善：其一，进一步增强画像快照与跨年度对比能力，提升画像历史可追溯性；其二，在保证边界可控的前提下完善问答检索链路，提高系统内知识辅助的覆盖范围；其三，结合更真实的院系业务数据进一步优化推荐规则与反馈闭环；其四，探索学院看板的离线聚合与缓存机制，以提升在更大数据规模下的稳定性和响应效率。相关方向与大语言模型辅助应用、知识图谱扩展和智能可视分析的发展趋势具有一定一致性，但在实际推进时仍应坚持面向真实业务边界的渐进式演化(Zhao et al. 2023)(Ji et al. 2021)(骆昱宇 et al. 2024)。
...
| 智能问答 | /api/ai-assistant/portrait-qa/; /api/ai-assistant/dify-chat/ | 返回受控式问答结果及可选模型扩展结果 |
...
<div id="ref-research_productivity_2011" class="csl-entry">

Abramo, Giovanni, and Ciriaco Andrea D’Angelo. 2014. “How Do You Define and Measure Research Productivity?” *Scientometrics* 101 (2): 1129–44. <https://doi.org/10.1007/s11192-014-1269-8>.

<div id="ref-next_gen_recsys_2007" class="csl-entry">
...
<div id="ref-expert_finding_models_2006" class="csl-entry">

Balog, Krisztian, Leif Azzopardi, and Maarten de Rijke. 2006. “Formal Models for Expert Finding in Enterprise Corpora.” In *Proceedings of the 29th Annual International ACM SIGIR Conference on Research and Development in Information Retrieval*.

<div id="ref-h_index_2005" class="csl-entry">
...
<div id="ref-expert_finding_systems_2019" class="csl-entry">

Husain, Omayma, Naomie Salim, Rose Alinda Alias, Samah Abdelsalam, and Alzubair Hassan. 2019. “Expert Finding Systems: A Systematic Review.” *Applied Sciences* 9 (20): 4250. <https://doi.org/10.3390/app9204250>.

<div id="ref-kg_survey_foreign_2021" class="csl-entry">

Ji, Shaoxiong, Shirui Pan, Erik Cambria, Pekka Marttinen, and Philip S. Yu. 2021. “A Survey on Knowledge Graphs: Representation, Acquisition and Applications.” *IEEE Transactions on Neural Networks and Learning Systems*.

<div id="ref-dpr_2020" class="csl-entry">

Karpukhin, Vladimir, Barlas Oguz, Sewon Min, Patrick Lewis, Ledell Wu, Sergey Edunov, Danqi Chen, and Wen-tau Yih. 2020. “Dense Passage Retrieval for Open-Domain Question Answering.” In *Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing (EMNLP)*, 6769–81.

<div id="ref-visual_analytics_2008" class="csl-entry">

Keim, Daniel, Gennady Andrienko, Jean-Daniel Fekete, Carsten Görg, Jörn Kohlhammer, and Guy Melançon. 2008. “Visual Analytics: Definition, Process, and Challenges.” In *Information Visualization*, 154–75. Springer.

<div id="ref-rag_2021" class="csl-entry">
...
<div id="ref-recommender_systems_2012" class="csl-entry">

Lü, Linyuan, Matúš Medo, Chi Ho Yeung, Yi-Cheng Zhang, Zi-Ke Zhang, and Tao Zhou. 2012. “Recommender Systems.” *Physics Reports* 519 (1): 1–49.

<div id="ref-scientific_impact_2020" class="csl-entry">

Siudem, Grzegorz, Barbara Zogala-Siudem, Anna Cena, and Marek Gagolewski. 2020. “Three Dimensions of Scientific Impact.” *Proceedings of the National Academy of Sciences* 117 (25): 13896–900. <https://doi.org/10.1073/pnas.2001064117>.

<div id="ref-arnetminer_2008" class="csl-entry">

Tang, Jie, Jing Zhang, Limin Yao, Juanzi Li, Li Zhang, and Zhong Su. 2008. “ArnetMiner: Extraction and Mining of Academic Social Networks.” In *Proceedings of the 14th ACM SIGKDD International Conference on Knowledge Discovery and Data Mining*, 990–98.

<div id="ref-llm_survey_2023" class="csl-entry">
...
<div id="ref-recsys_survey_cn_2022" class="csl-entry">

于蒙, 何文涛, 周绪川, 崔梦天, 吴克奇, and 周文杰. 2022. “推荐系统综述.” *计算机应用* 42 (6): 1898–1913.

<div id="ref-kg_construction_2022" class="csl-entry">

张吉祥, 张祥森, 武长旭, and 赵增顺. 2022. “知识图谱构建技术综述.” *计算机工程* 48 (3): 23–37.

<div id="ref-sci_policy_2020" class="csl-entry">

教育部, and 科技部. 2020. “关于规范高等学校sci论文相关指标使用 树立正确评价导向的若干意见.”

<div id="ref-academic_kg_service_2018" class="csl-entry">

汤庸, 陈国华, 贺超波, and 彭博. 2018. “知识图谱及其在学术信息服务领域的应用.” *华南师范大学学报（自然科学版）* 50 (5): 110–19.

<div id="ref-scholar_visualization_2020" class="csl-entry">

王杨, 余敏槠, 单桂华, and 陆忠华. 2020. “学者学术状态及竞争力可视化系统.” *计算机系统应用* 29 (8): 48–57. <https://doi.org/10.15888/j.cnki.csa.007597>.

<div id="ref-edu_digitalization_2023" class="csl-entry">

祝智庭, and 胡姣. 2022. “教育数字化转型：面向未来的教育‘转基因’工程.” *开放教育研究* 28 (5): 12–19. <https://doi.org/10.13966/j.cnki.kfjyyj.2022.05.002>.

<div id="ref-qa_system_survey_2023" class="csl-entry">

闫悦, 郭晓然, 王铁君, 饶强, and 王铠杰. 2023. “问答系统研究综述.” *计算机系统应用* 32 (8): 1–18.

<div id="ref-intelligent_visual_analytics_2024" class="csl-entry">

骆昱宇, 秦雪迪, 谢宇鹏, and 李国良. 2024. “智能数据可视分析技术综述.” *软件学报* 35 (1): 356–404.

<div id="ref-kg_general_survey" class="csl-entry">

黄恒琪, 于娟, 廖晓, and 席运江. 2019. “知识图谱研究综述.” *计算机系统应用* 28 (6): 1–12. <https://doi.org/10.15888/j.cnki.csa.006915>.
